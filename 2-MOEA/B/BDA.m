function [Z,A,Distance] = BDA(X_src,Y_src,X_tar,Y_tar,options)
    % Inputs:
...
%          = knn_model.predict(X_tar);
%     end

    %% 从此处开始计算，根据支配关系来给源域数据和目标域数据来分类，非支配解作为第一类，支配解作为第二类
    %%% Mc
    N = 0;
    if ~isempty(Y_tar) && length(Y_tar)==nt
        for c = reshape(unique(Y_src),1,C)
            e = zeros(n,1);
            if strcmp(mode,'W-BDA')
                Ps = length(find(Y_src==c)) / length(Y_src);
                Pt = length(find(Y_tar == c)) / length(Y_tar);
                alpha = Pt / Ps;
                mu = 1;
            else
                alpha = 1;
            ...
            e(ns+find(Y_tar==c)) = -alpha / length(find(Y_tar==c));
            e(isinf(e)) = 0;
            N = N + e*e';
        end
    end
...
%     Zt = Z(:,size(X_src,1)+1:end)';
    ...
    %% 不需要剩下的计算准确率的部分了，上面将原始空间的数据映射到隐空间里即可
