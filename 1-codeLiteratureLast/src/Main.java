...
public class Main {
    public static void main(String[] args) throws IOException, MWException {
        ...
        String m = read_original_TAOG(taog_sum,taog_list);
        ...
        for(int z = 0; z < 10; z++){
            ...
            for (int i = 0; i < 8; i++){
                ...
                //进化
                pop.evolution(taog_list, taog_sum, tmp);
                //计算每个环境下的pareto解
                pop.output_PF(pf);
            }

            //开始前将存放帕累托解的文件夹清空
            String deleteParetoPath = "F:\\Dissertation\\newPfResult\\"+ m +"\\NSGAII\\original\\run"+ (z+1);
            Problem.deleteDir(deleteParetoPath);

            //把八个环境下的pf保存下来
            for (int i = 0; i < 8; i++) {

                String path = "F:\\Dissertation\\newPfResult\\"+ m +"\\NSGAII\\original\\run"+ (z+1) +"\\OriPF"+i+".txt";
    ...
    public static String read_original_TAOG(TAOG taog_sum,List<TAOG> taog_list) throws IOException {

        int[] TAOG_name = {1,3};
        String m = TAOG_name[0] + "";
        for (int i = 1; i < TAOG_name.length; i++) {
            m =  m + "," + TAOG_name[i];
        }
