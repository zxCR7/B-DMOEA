...
public class MainRVEA {
    public static void main(String[] args) throws IOException, MWException {
        ...
        for (int i = 0; i < 8; i++){
            ...
            //先是存储初代种群数据
            String xlsFile = "F:\\RVEAiGNGOutput\\1\\parent.xls";
            ...
            for (int j = 1; j < 50; j++){
                ...
                File file = new File("F:\\RVEAiGNGOutput\\1\\parent" + j +".xls");
                ...
                for (int k = 0; k < indexList.size(); k=k+2){
                    popTest.pt_list.add(all_pop.get(indexList.get(k)).get(indexList.get(k + 1)));
                }

                //popTest.produce_qt_list_RVEA(taog_list,taog_sum);
                all_pop.add(popTest.qt_list);

                //将产生的子代个体的目标函数值、约束违反值以及index写到excel中保存下来
                String xlsFile1 = "F:\\RVEAiGNGOutput\\1\\offspring"+ j +".xls";

                HSSFWorkbook wb2 = new HSSFWorkbook();
                HSSFSheet sheet2 = wb2.createSheet("test");

                for (int k = 0; k < popTest.qt_list.size(); k++){
                    HSSFRow row2 = sheet2.createRow(k);
                    //创建单元格设值
                    row2.createCell(0).setCellValue(popTest.qt_list.get(k).objectives.get(0));
                    row2.createCell(1).setCellValue(popTest.qt_list.get(k).objectives.get(1));
                    row2.createCell(2).setCellValue(popTest.qt_list.get(k).objectives.get(2));
            ...
            String path = "F:\\pfResult\\RVEAiGNG\\original\\"+"OriPF"+i+".txt";
