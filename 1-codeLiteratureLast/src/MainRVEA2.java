import BDAmmd.DisToPf;
import GetBDAdistance.BDADistance;
...
public class MainRVEA2 {
    public static void main(String[] args) throws IOException, MWException {
        ...
        for (int z = 0; z < 10; z++) {
            System.out.println(z + " experiment is beginning......");

//            double IGD[] = new double[8];
//            //开始前将存放pf的文件夹清空
//            String deletePathDir = "F:\\pfResult\\RVEAiGNG\\BDA\\" + z;
//            Problem.deleteDir(deletePathDir);

            //使用BDA
            //首先是初始环境
            //建立一个集合存储迭代过程中的当前环境下的种群,这个种群存储的都是引导种群(这个种群里可以认为是进化完成后的种群，比较好的种群),用该种群去参与构建变换矩阵
            List<List<Individual>> good_population = new ArrayList<>();
            //存储每个环境下种群迭代后的种群的分类标签
            List<List<Integer>> good_population_label = new ArrayList<>();
            //存储每个环境下的全部种群，包括每一次进化产生的种群
            List<ArrayList<Individual>> pf = new ArrayList<ArrayList<Individual>>();

            for (int i = 0; i < 1; i++){

                //开始前将存放所有解的文件夹清空
                String deleteALLPath = "F:\\pfResult\\RVEA\\BDA\\run"+ (z+1) + "\\"+ + i +"\\all";
                Problem.deleteDir(deleteALLPath);

                //开始前将存放pf的文件夹清空
                String deletePath = "F:\\RVEAiGNGOutput\\3";
                ...
                //每一代的个体
                List<List<Individual>> all_pop = new ArrayList<>();
                ...
                all_pop.add(pop.pt_list);

                //先是存储初代种群数据
                String xlsFile = "F:\\RVEAiGNGOutput\\3\\parent.xls";

                //针对初代种群将其目标函数值写进EXCEL
                ...
                for (int j = 0; j < pop.pt_list.size(); j++){
                    ...
                    //创建单元格设值
                    row1.createCell(0).setCellValue(pop.pt_list.get(j).objectives.get(0));
                    row1.createCell(1).setCellValue(pop.pt_list.get(j).objectives.get(1));
                    row1.createCell(2).setCellValue(pop.pt_list.get(j).objectives.get(2));
                    row1.createCell(3).setCellValue(0);
                    row1.createCell(4).setCellValue(j);
                }
                ...
                //往文件夹里创建一个空文本文档，Matlab那边检测到这个文档，则继续进行
                String flagFilePath = "F:\\RVEAiGNGOutput\\3\\Flag.txt";
                ...
                for (int j = 1; j < 31; j++){
                    ...
                    File file = new File("F:\\RVEAiGNGOutput\\3\\parent" + j +".xls");

                    //这一步确定父代
                    while (true){
                        if (!flagFile.exists()){
                            indexList.clear();
                            HSSFWorkbook wb1 = new HSSFWorkbook(new FileInputStream(file));
                            //读取第一页,一般一个excel文件会有三个工作表，这里获取第一个工作表来进行操作
                            HSSFSheet sheet1 = wb1.getSheetAt(0);
                            for(int k = 0; k < sheet1.getLastRowNum()+1; k++) {
                                //创建一个行对象
                                HSSFRow row1 = sheet1.getRow(k);
                                //把每一行里的index遍历出来(包括代数和在某一代里的序号)
                                HSSFCell cell1 = row1.getCell(3);
                                indexList.add((int) cell1.getNumericCellValue());
                                HSSFCell cell2 = row1.getCell(4);
                                indexList.add((int)cell2.getNumericCellValue());
                            }
                            break;
                        }
                    }

//                //获取到index之后，将这些个体作为父代种群，来进行交叉变异生成子代种群
//                //NSGAII popTest = new NSGAII(population_num, 50);
//                for (int k = 0; k < indexList.size(); k=k+2){
//                    pop.pt_list.add(all_pop.get(indexList.get(k)).get(indexList.get(k + 1)));
//                }
//
//                //pop.produce_qt_list_RVEA(taog_list,taog_sum);
//                all_pop.add(pop.qt_list);

                    pop.produce_qt_list_RVEA(taog_list, taog_sum, indexList, all_pop);

                    List<Individual> zhongzhuan = new ArrayList<>();
                    for (int g = 0; g < pop.qt_list.size(); g++) {
                        zhongzhuan.add(pop.qt_list.get(g));
                    }
                    all_pop.add(zhongzhuan);

                    //将产生的子代个体的目标函数值以及index写到excel中保存下来
                    String xlsFile1 = "F:\\RVEAiGNGOutput\\3\\offspring"+ j +".xls";

                    HSSFWorkbook wb2 = new HSSFWorkbook();
                    HSSFSheet sheet2 = wb2.createSheet("test");

                    for (int k = 0; k < pop.qt_list.size(); k++){
                        HSSFRow row2 = sheet2.createRow(k);
                        //创建单元格设值
                        row2.createCell(0).setCellValue(pop.qt_list.get(k).objectives.get(0));
                        row2.createCell(1).setCellValue(pop.qt_list.get(k).objectives.get(1));
                        row2.createCell(2).setCellValue(pop.qt_list.get(k).objectives.get(2));
                        row2.createCell(3).setCellValue(j);
                        row2.createCell(4).setCellValue(k);
                    }

                    FileOutputStream fOut2 = new FileOutputStream(xlsFile1);
                    wb2.write(fOut2);
                    fOut2.flush();
                    fOut2.close();
                ...
                for (int j = 0; j < all_pop.size(); j++){
                    now_all_solutions.addAll(all_pop.get(j));
                }

                //在这里就进行快速非支配排序
                //这里想尽可能地覆盖好解和坏解，从当前环境的迭代过程产生的所有解中，随机采样一些，对采样点进行非支配排序
//                List<Integer> sampleIndex = new ArrayList<>();
//                Individual.getRandomNumListNew(population_num,0,now_all_solutions.size(),sampleIndex);
//                NSGAII samplePop = new NSGAII(population_num, evolution_num);
//                for (int j = 0; j < sampleIndex.size(); j++){
//                    samplePop.pt_list.add(now_all_solutions.get(sampleIndex.get(j)));
//                }
                NSGAII samplePop = new NSGAII(population_num, evolution_num);
                samplePop.pt_list.addAll(samplePop.init_solution(taog_list, taog_sum, state));

                //预搜索
                samplePop.evolutionPresearch(taog_list, taog_sum, 3);
                samplePop.fast_non_dominated_sort(samplePop.pt_list);
                List<Individual> temp_good_population = new ArrayList<>();
                List<Integer> temp_good_population_label = new ArrayList<>();
                for (int j = 0; j < samplePop.fronts.size(); j++){
                    if (j == 0){
                        for (int k = 0; k < samplePop.fronts.get(j).size(); k++){
                            temp_good_population.add(samplePop.fronts.get(j).get(k));
                            temp_good_population_label.add(1);
                        }
                    }else {
                        for (int k = 0; k < samplePop.fronts.get(j).size(); k++){
                            temp_good_population.add(samplePop.fronts.get(j).get(k));
                            temp_good_population_label.add(2);
                        }
                    }
                }

                good_population.add(temp_good_population);
                good_population_label.add(temp_good_population_label);


                pop.pt_list.clear();
                pop.pt_list.addAll(pop.qt_list);

                pop.output_PF(pf);

                //保存本次实验的所有的解
                String pathAll = "F:\\pfResult\\RVEA\\BDA\\run"+ (z+1) +"\\"+ i +"\\all\\"+"all"+i+".txt";
...
//                double igd = Problem.IGD(pf.get(i), truePF);
//                IGD[i] = igd;
...
//
//                String path = "F:\\pfResult\\RVEAiGNG\\BDA\\"+ z +"\\OriPF"+i+".txt";
...
//                fileWrite.close();
            ...
            for (int i = 1; i < 8; i++){
                ...
                MWNumericArray gCovHistoryLabel = new MWNumericArray();
                MWNumericArray gCovNowLabel = new MWNumericArray();
                //MMD距离
                BDADistance mmd = new BDADistance();
                //变换矩阵W
                //GetW calW = new GetW();
                //保存mmd距离的集合
                ...
                Object[] array_label_now;
                Object[] array_label_history;

                //根据支配关系来分类，非支配解作为第一类，支配解作为第二类
                //先进化3代做一个预搜索
                pop.evolutionPresearch(taog_list,taog_sum,3);
                pop.fast_non_dominated_sort(pop.rt_list);

                //存储当前环境下的种群
                List<Double> list_temp_now = new ArrayList<>();
                //存储当前环境下解的标签
                List<Integer> label_now = new ArrayList<>();
                List<Individual> paiXu = new ArrayList<>();
                int labeflag = 0;
                forLabel:
                for (int j = 0; j < pop.fronts.size(); j++){
                    //第一层的个体为第一类
                    if (j == 0){
                        for (int k = 0; k < pop.fronts.get(j).size(); k++){
                            list_temp_now.add(pop.fronts.get(j).get(k).objectives.get(0));
                            list_temp_now.add(pop.fronts.get(j).get(k).objectives.get(1));
                            list_temp_now.add(pop.fronts.get(j).get(k).objectives.get(2));
                            label_now.add(1);
                            paiXu.add(pop.fronts.get(j).get(k));
                            labeflag ++;
                            if (labeflag == 150){
                                break forLabel;
                            }
                ...
                array_label_now = label_now.toArray();

                //将数组转换为矩阵，这是当前环境下的种群
                gCovNow = new MWNumericArray(array_temp_now, MWClassID.DOUBLE);
                //这是当前环境下的种群的分类标签
                gCovNowLabel = new MWNumericArray(array_label_now, MWClassID.DOUBLE);

                //这里就将预搜索得到的种群加到好解集合中，用来构建W矩阵
                good_population.add(paiXu);
                good_population_label.add(label_now);

                //存储跟每个历史环境数据计算得到的变换矩阵A
                List<MWNumericArray> all_transformA = new ArrayList<>();
                //计算当前环境跟历史环境下种群的距离

                for (int k = 0; k < i; k++){
                    //接下来就进行迁移学习，找到合适的种群来用作初始化种群
                    //要比较前面n-1个环境下的种群分布跟当前环境下种群分布的差异

                    //首先获取到历史环境下的种群，从good_population中选择
                    List<Double> list_temp_history = new ArrayList<>();
                    //List<Integer> label_history = new ArrayList<>();
                    for (int l = 0; l < good_population.get(k).size(); l++){
                        list_temp_history.add(good_population.get(k).get(l).objectives.get(0));
                        list_temp_history.add(good_population.get(k).get(l).objectives.get(1));
                        list_temp_history.add(good_population.get(k).get(l).objectives.get(2));
                    }
                    array_temp_history = list_temp_history.toArray();
                    array_label_history = good_population_label.get(k).toArray();

                    //将数组转换成矩阵，这就是某个历史环境下的种群
                    gCovHistory = new MWNumericArray(array_temp_history, MWClassID.DOUBLE);
                    //这是某个历史环境下的分类标签
                    gCovHistoryLabel = new MWNumericArray(array_label_history, MWClassID.DOUBLE);

                    Object[] mmdResult = null;
                    //求当前环境同历史环境的距离,现在不仅要传原始种群，还要传已经分类好的标签
                    mmdResult = mmd.GetBDAdistance(3,gCovHistory,gCovHistoryLabel,gCovNow,gCovNowLabel);

                    MWNumericArray dataMMD = (MWNumericArray) mmdResult[2];					// 第一个输出参数
                    MWNumericArray transformA = (MWNumericArray) mmdResult[1];

                    all_transformA.add(transformA);

                    double[][] MMDDouble = (double[][]) dataMMD.toDoubleArray();

                    mmdList.add(MMDDouble[0][0]);
                }

                Double minNum = Collections.min(mmdList);

                //确定距离最小的环境，即最相似的环境
                int minIndex = mmdList.indexOf(minNum);

                System.out.println("selected environment: " + minIndex);

                //最相似环境下的种群
                List<Double> list_temp_similar = new ArrayList<>();
                for (int k = 0; k < good_population.get(minIndex).size(); k++){
                    list_temp_similar.add(good_population.get(minIndex).get(k).objectives.get(0));
                    list_temp_similar.add(good_population.get(minIndex).get(k).objectives.get(1));
                    list_temp_similar.add(good_population.get(minIndex).get(k).objectives.get(2));
                }
                ...
                //创建一个对象用来表示最终的经过迁移学习和单目标优化得到的种群pop_final,假设最后针对这个种群利用NSGAII进化30代
                NSGAII pop_final = new NSGAII(population_num, 30);

//                //这个时间太久了，对pf上的点进行采样，对部分点进行单目标优化，暂定20个点，每个点选择10个距离他最近的
//
//                List<Integer> pfSample = new ArrayList<>();
//                Individual.getRandomNumListNew(15,0,pf.get(minIndex).size(),pfSample);
//                List<Individual> sort_crowding_distance = pop.sort_individual_by_crowding_distance(pf.get(minIndex));


//            //针对最相似环境下的PF上的每一个解，将当前环境下的种群作为这个单目标优化问题的初始种群，个体离PF上的解的距离作为适应度值
                NSGAII popSingleObj = new NSGAII(population_num, evolution_num);
                for (int k = 0; k < pf.get(minIndex).size(); k++){
//                for (int k = 0; k < pfSample.size(); k++){
                    ...
                    list_pf_similar.add(pf.get(minIndex).get(k).objectives.get(0));
                    list_pf_similar.add(pf.get(minIndex).get(k).objectives.get(1));
                    list_pf_similar.add(pf.get(minIndex).get(k).objectives.get(2));
//                    list_pf_similar.add(pf.get(minIndex).get(pfSample.get(k)).objectives.get(0));
//                    list_pf_similar.add(pf.get(minIndex).get(pfSample.get(k)).objectives.get(1));
//                    list_pf_similar.add(pf.get(minIndex).get(pfSample.get(k)).objectives.get(2));
                    //这个是根据拥挤度距离从帕累托面上采样一些解来进行单目标优化
//                    list_pf_similar.add(sort_crowding_distance.get(k).objectives.get(0));
//                    list_pf_similar.add(sort_crowding_distance.get(k).objectives.get(1));
//                    list_pf_similar.add(sort_crowding_distance.get(k).objectives.get(2));
                    ...
                    DisToPf disToPf = new DisToPf();

                    //result里存储的是计算距离之后，排序的结果，接下来要基于这个排序去选择解作为父代种群去迭代
                    distance_object = disToPf.BDAmmd(2,gCovHistory,gCovSingleObj,gCovPf,all_transformA.get(minIndex));
                    ...
                    for (int m = 0; m < 3; m++){
                        ...
                        distance_object = disToPf.BDAmmd(2, gCovHistory, gCovNow,gCovPf, all_transformA.get(minIndex));
                ...
                for (int j = 0; j < pop_final.pt_list.size(); j++){
                    HSSFRow row1 = sheet.createRow(j);
                    //创建单元格设值
                    row1.createCell(0).setCellValue(pop_final.pt_list.get(j).objectives.get(0));
                    row1.createCell(1).setCellValue(pop_final.pt_list.get(j).objectives.get(1));
                    row1.createCell(2).setCellValue(pop_final.pt_list.get(j).objectives.get(2));
                    row1.createCell(3).setCellValue(0);
                    row1.createCell(4).setCellValue(j);
                }
...
//                    //获取到index之后，将这些个体作为父代种群，来进行交叉变异生成子代种群
//                    pop_final.pt_list.clear();
//                    for (int k = 0; k < indexList.size(); k=k+2){
//                        pop_final.pt_list.add(all_pop.get(indexList.get(k)).get(indexList.get(k + 1)));
//                    }
//
//                    //pop_final.produce_qt_list_RVEA(taog_list,taog_sum);
//                    all_pop.add(pop_final.qt_list);

                    pop_final.produce_qt_list_RVEA(taog_list, taog_sum, indexList, all_pop);

                    List<Individual> zhongzhuan = new ArrayList<>();
                    for (int g = 0; g < pop_final.qt_list.size(); g++) {
                        zhongzhuan.add(pop_final.qt_list.get(g));
                    }
                    all_pop.add(zhongzhuan);
                    ...
                    for (int k = 0; k < pop_final.qt_list.size(); k++){
                        HSSFRow row2 = sheet2.createRow(k);
                        //创建单元格设值
                        row2.createCell(0).setCellValue(pop_final.qt_list.get(k).objectives.get(0));
                        row2.createCell(1).setCellValue(pop_final.qt_list.get(k).objectives.get(1));
                        row2.createCell(2).setCellValue(pop_final.qt_list.get(k).objectives.get(2));
                        row2.createCell(3).setCellValue(j);
                        row2.createCell(4).setCellValue(k);
                    }
...
//                good_population_label.add(temp_good_population_label);
                ...
                pop_final.pt_list.clear();
                pop_final.pt_list.addAll(pop_final.qt_list);

                //输出其帕累托最优解
                pop_final.output_PF(pf);
...
//                fileWrite.close();
            ...
            //开始前将存放帕累托解的文件夹清空
            String deleteParetoPath = "F:\\pfResult\\RVEA\\BDA\\run"+ (z+1) +"\\pf";
...
//                normalization(pf1.get(i),max,min);
                ...
                String path = "F:\\pfResult\\RVEA\\BDA\\run"+ (z+1) +"\\pf\\OriPF"+i+".txt";
                File file = new File(path);
                if(!file.exists()){
                    file.createNewFile();
                }
...
//                sum += IGD[i];
//            }
//            System.out.println("Mean IGD:   " + sum/8);
//
//            for (int i = 0; i < 8; i++) {
//                double max[]= {Double.MIN_VALUE,Double.MIN_VALUE,Double.MIN_VALUE};
//                double min[]= {Double.MAX_VALUE,Double.MAX_VALUE,Double.MAX_VALUE};
...
//    public static void getMaxAndMinObjectives(List<ArrayList<Individual>> pf,double max[],double min[]) {
