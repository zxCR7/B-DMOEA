...
//            //开始前将存放pf的文件夹清空
//            String deletePath = "F:\\pfResult\\IBEA\\BDA\\" + z;
//            Problem.deleteDir(deletePath);
            ...
            for (int i = 0; i < 1; i++){
                ...
                //开始前将存放所有解的文件夹清空
                String deletePath = "F:\\pfResult\\IBEA\\BDA\\run"+ (z+1) + "\\"+ + i +"\\all";
                Problem.deleteDir(deletePath);
                ...
                IBEA samplePop = new IBEA(population_num, evolution_num);
                ...
                //保存本次实验的所有的解
                String pathAll = "F:\\pfResult\\IBEA\\BDA\\run"+ (z+1) +"\\"+ i +"\\all\\"+"all"+i+".txt";
...
//
//                String path = "F:\\pfResult\\IBEA\\BDA\\"+ z + "\\OriPF"+i+".txt";
...
//                fileWrite.close();
            ...
            for (int i = 1; i < 8; i++){
                ...
                //创建一个对象用来表示最终的经过迁移学习和单目标优化得到的种群pop_final,假设最后针对这个种群利用IBEA进化30代
                IBEA pop_final = new IBEA(population_num, 30);

//                //此处应再结合拥挤度距离来采样，因为最后在帕累托面上的解太多了，要结合拥挤度距离来采样
...
//            Individual.getRandomNumListNew(10,0,pf.get(minIndex).size(),pfSample);
...
//            for (int k = 0; k < pf.get(minIndex).size(); k++){
                ...
                IBEA popSingleObj = new IBEA(population_num, evolution_num);
                for (int k = 0; k < pf.get(minIndex).size(); k++){
...
//                //针对每一个解，要去做相应的进化，那么就有创建一个集合把迭代过程中的解都保留下来
//                List<List<Individual>> every_gen = new ArrayList<>();
//                every_gen.add(popSingleObj.pt_list);
...
//                    list_pf_similar.add(sort_crowding_distance.get(k).objectives.get(2));
                    ...
                    for (int m = 0; m < 3; m++){
                        ...
                        for (int n = 0; n < population_num; n++){
                            ...
                            //随机选择两个父本进行交叉变异产生一个较好个体 进行非支配排序，对于EIBEA来说，constraintKind为0
                        ...
                        IBEA aaa = new IBEA(population_num, evolution_num);
                        int flag = 0;
                        forOut:
...
//                    //将距离最近的10个点添加到初始种群中
//                    for (int m = 0; m < 10; m++){
//                        pop_final.pt_list.add(popSingleObj.pt_list.get(distance_index[0][m] - 1));
//                    }
            ...
            //开始前将存放帕累托解的文件夹清空
            String deleteParetoPath = "F:\\pfResult\\IBEA\\BDA\\run"+ (z+1) +"\\pf";
...
//                normalization(pf1.get(i),max,min);
                ...
                String path = "F:\\pfResult\\IBEA\\BDA\\run"+ (z+1) +"\\pf\\OriPF"+i+".txt";
                File file = new File(path);
                if(!file.exists()){
                    file.createNewFile();
                }
...
//            }
//
//            System.out.println(z+" experiment's Mean IGD is   " + sum/8);
