    ...
    public static void main(String[] args) throws IOException, MWException {
        ...
        for(int z = 0; z < 10; z++){
            ...
            for (int i = 0; i < 1; i++){
                ...
                //开始前将存放所有解的文件夹清空
                String deletePath = "F:\\pfResult\\NSGAII\\BDA\\run"+ (z+1) + "\\"+ + i +"\\all";
                ...
                //保存本次实验的所有的解
                String pathAll = "F:\\pfResult\\NSGAII\\BDA\\run"+ (z+1) +"\\"+ i +"\\all\\"+"all"+i+".txt";
            ...
            for (int i = 1; i < 8; i++){
                ...
                //System.out.println("Time cost is "+(System.currentTimeMillis()-allTimeBegin)/1000/60+"min!");
                for (int k = 0; k < i; k++){
...
//            for (int k = 0; k < 15; k++){
            ...
            //开始前将存放帕累托解的文件夹清空
            String deleteParetoPath = "F:\\pfResult\\NSGAII\\BDA\\run"+ (z+1) +"\\pf";
            Problem.deleteDir(deleteParetoPath);

            //把八个环境下的pf保存下来
            for (int i = 0; i < 8; i++) {

                String path = "F:\\pfResult\\NSGAII\\BDA\\run"+ (z+1) +"\\pf\\OriPF"+i+".txt";
