    ...
    public static void main(String[] args) throws IOException, MWException {
        ...
        for (int z = 0; z < 10; z++) {
            ...
            for (int i = 0; i < 1; i++){
                ...
                //开始前将存放所有解的文件夹清空
                String deletePath = "F:\\pfResult\\IBEA\\TCA\\run"+ (z+1) + "\\"+ + i +"\\all";
                ...
                //保存本次实验的所有的解
                String pathAll = "F:\\pfResult\\IBEA\\TCA\\run"+ (z+1) +"\\"+ i +"\\all\\"+"all"+i+".txt";
...
//                double igd = Problem.IGD(pf1.get(i), truePF);
//                IGD[i] = igd;
//
...
//
//                String path = "F:\\pfResult\\IBEA\\TCA\\"+ z +"\\OriPF"+i+".txt";
...
//                    for (int l = 0; l < good_population1.get(k).size(); l++){
//                        list_temp_history.add(good_population1.get(k).get(l).objectives.get(0));
//                        list_temp_history.add(good_population1.get(k).get(l).objectives.get(1));
//                        list_temp_history.add(good_population1.get(k).get(l).objectives.get(2));
//                    }
...
//                    //求当前环境同历史环境的距离
//                    mmdResult = mmd.getMMD(1,gCovHistory,gCovNow);
//
//                    MWNumericArray dataMMD = (MWNumericArray) mmdResult[0];					// 第一个输出参数
...
//            Individual.getRandomNumListNew(10,0,pf1.get(minIndex).size(),pfSample);
...
//                list_pf_similar.add(pf1.get(minIndex).get(pfSample.get(k)).objectives.get(0));
//                list_pf_similar.add(pf1.get(minIndex).get(pfSample.get(k)).objectives.get(1));
//                list_pf_similar.add(pf1.get(minIndex).get(pfSample.get(k)).objectives.get(2));
...
//                //针对每个解做单目标优化后，将距离最小的那个解保留下来(即排在最前面的那个解)，将来填充到初始种群中
//                if (k < pf1.get(minIndex).size() - 1){
//                    pop_final.pt_list.add(popSingleObj.pt_list.get(0));
//                }else {
//                    //最后判断初始种群中的个体是否足够，不够的话，再进行填充
//                    if (pop_final.pt_list.size() < population_num){
//                        int buchong = population_num - pop_final.pt_list.size();
//                        for (int m = 0; m < buchong; m++){
//                            pop_final.pt_list.add(popSingleObj.pt_list.get(m));
//                        }
//                    }
//                }
//
...
//                fileWrite.close();
            ...
            //开始前将存放帕累托解的文件夹清空
            String deleteParetoPath = "F:\\pfResult\\IBEA\\TCA\\run"+ (z+1) +"\\pf";
...
//                normalization(pf1.get(i),max,min);
                ...
                String path = "F:\\pfResult\\IBEA\\TCA\\run"+ (z+1) +"\\pf\\OriPF"+i+".txt";
                File file = new File(path);
                if(!file.exists()){
                    file.createNewFile();
                }
