function [Population,net,V,Archive,scale,genFlag] = EnvironmentalSelection(Population,V,theta,net,params,Archive,Global,scale,zmin,genFlag)
...
Archive = UpdateArchive(Population,Archive,2*N);
ArcObj = Archive.objs;
...
[N,M]  = size(PopObj);
fr = 0.1;
% if ~mod(Global.gen,ceil(fr*Global.maxgen))&& Global.gen <= round(1*Global.maxgen)
%     w = net.w;
%     w = w.*scale;
%     w = w./sum(w,2);
...
if size(temp1,1) > 2&&isempty(find(isnan(temp1)==true))&& Global.gen <= round(1*Global.maxgen) && isempty(genFlag)
    [V,net,genFlag] = TrainGrowingGasNet(V,temp1,net,scale,params,Global,[[];ArcObj],genFlag,zmin);
end
NV     = size(V,1);
